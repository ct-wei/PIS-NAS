...
class Strategy(metaclass=ABCMeta):
    ...
    def build_model(self, ):
        if hasattr(self.cfg, 'model'):
            ...
            self.logger.info(self.super_model)
        else:
    ...
    def get_info_for_evolution(self, structure_info=None):
        ...
        for key in self._budget_values:
            kwargs = {}
            if key in {'flops', 'max_feature','bottleneck_entropy'}:
        ...
        if model_info['is_satify_budget']:
            ...
            if (self.cfg.score.type == 'deepmad' or 'erfnas') and depth_penalty_ratio != 0:
                depth_list_every_block = []
                for block in model_info["structure_info"]:
                    if 'L' in block:
                        depth_list_every_block.append(int(block['L']))
                    elif 'n' in block:
                        depth_list_every_block.append(int(block['n']))
                ...
                depth_scales = self.cfg.score.get('depth_scales', None)
                if depth_scales is not None:
                    depth_list_every_block = [d / s for d, s in zip(depth_list_every_block, depth_scales)]
                # print(depth_list_every_block)
