...
class ConvKXBNRELU(ConvKXBN):
    ...
    def get_bottleneck_entropy(self,in_resolution,**kwarg):
        ...
        out_resolution=in_resolution/self.stride
        
        conv=(np.log(np.sqrt(self.in_channels ** kwarg["alpha1"])) +
...
class BaseSuperBlock(nn.Module):
    ...
    def get_bottleneck_entropy(self,in_resolution,**kwarg):
        bottleneck_entropy=inf
        for block in self.block_list:
            bottleneck_entropy=min(block.get_bottleneck_entropy(in_resolution,**kwarg),bottleneck_entropy) 
            
        return bottleneck_entropy
