...
class ResK1KXK1(nn.Module):
    ...
    def __init__(self,
        ...
        if self.in_channels != self.out_channels or self.force_resproj:
            ...
            self.flops = self.flops + self.residual_proj.get_flops(
                1.0) + self.out_channels / self.stride**2
    ...
    def get_bottleneck_entropy(self,in_resolution,**kwarg):
        ...
        # conv1=(np.log(np.sqrt(self.in_channels ** kwarg["alpha1"])) +
        #       np.log(np.sqrt(1 ** (2 * kwarg["alpha2"]))) )*(np.log(np.sqrt(out_resolution ** (2 * kwarg["alpha2"]))) +
        #       np.log(np.sqrt(self.bottleneck_channels ** kwarg["alpha1"]))
        #       )
  
        out_resolution=out_resolution/self.stride
        conv2=(np.log(np.sqrt(self.bottleneck_channels ** kwarg["alpha1"])) +
            np.log(np.sqrt(self.kernel_size ** (2 * kwarg["alpha2"]))) +
            np.log(np.sqrt(out_resolution ** (2 * kwarg["alpha2"]))) +
            np.log(np.sqrt(self.bottleneck_channels ** kwarg["alpha1"]))
            )
        ...
        # conv2=(np.log(np.sqrt(self.bottleneck_channels ** kwarg["alpha1"])) +
        #     np.log(np.sqrt(self.kernel_size ** (2 * kwarg["alpha2"]))) )*(np.log(np.sqrt(out_resolution ** (2 * kwarg["alpha2"]))) +
        #     np.log(np.sqrt(self.bottleneck_channels ** kwarg["alpha1"]))
        #     )

        out_resolution=out_resolution
        conv3=(np.log(np.sqrt(self.bottleneck_channels ** kwarg["alpha1"])) +
            np.log(np.sqrt(self.kernel_size ** (2 * kwarg["alpha2"]))) +
            np.log(np.sqrt(out_resolution ** (2 * kwarg["alpha2"]))) +
