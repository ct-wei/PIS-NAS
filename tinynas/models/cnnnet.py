...
class CnnNet(Model):
    ...
    def get_bottleneck_entropy(self, **kwarg):
        resolution=kwarg['resolution']
        kwarg["alpha1"]=1
        kwarg["alpha2"]=1
        ...
        the_res = resolution
        for block in self.block_list:
            # print(block.get_bottleneck_entropy(the_res,**kwarg))
            bottleneck_entropy = min(block.get_bottleneck_entropy(the_res,**kwarg),bottleneck_entropy)
            the_res /= block.stride
            
        return float(- bottleneck_entropy)
